# Hi there 👋 I'm Nimesha Premaraja

🚀 **Cloud & DevOps Engineer | Infrastructure Automation | DevSecOps**

Experienced DevOps Engineer 5+ years of hands-on experience in software development and streamlining delivery processes, as demonstrated by a successful track record. Specializing in Cloud computing (AWS, GCP), Infrastructure as Code (Terraform), CI/CD, Containerization and Orchestration, Monitoring and Logging, Networking, Database management and scripting empowered using modern AI and AI Agentic capabilities.

---

# ☁️ Cloud & DevOps Stack

- **Cloud and Infrastructure:** AWS, GCP, Linux, Kubernetes, Helm, Docker, Terraform, Ansible
- **Programming and Scripting:** Python, Bash/Shell, PowerShell
- **Automation and CI/CD:** GitHub Actions, ArgoCD, Jenkins, Yaml
- **Monitoring:** Grafana, Prometheus, ELK Stack, AWS CloudWatch
- **Networking:** DNS, DHCP, Firewall, CDN, TCP/IP, HTTP/S
- **Security:** DevSecOps, IAM, SOC 2, Cloud Security
- **Other Tools:** Jira, Agile, GitHub, Trivy, MySQL

---

# 📚 Currently Learning

* AI and AI Agentic DevOps Automation
* AWS Advanced Architecture
* Advanced Kubernetes Concepts
* Cloud Security & DevSecOps Automation
* Platform Engineering Practices

---

# 🤝 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?logo=linkedin)](https://www.linkedin.com/in/nimesha-premaraja-6103a0191)

[![Email](https://img.shields.io/badge/Email-Contact-red?logo=gmail)](mailto:nimeshadilshanpm@gmail.com)
